...
t11 = -Tp/6;
delta = 2*sind(alpha)/inT/fd;
resSig = ModulatedData'.*( pulse1(t11,tau,t11+Tp/2,Tp,t,fs,delta) - pulse2(t11-Tp/4,tau,t11+Tp/4,Tp,t,fs,delta) );

for i=2:arrayNum
    t1i = (((i-1)*beta*z*sind(theta))/pi-1/3)/2*Tp;
    Sigi = ModulatedData'.*( pulse1(t1i,tau,t1i+Tp/2,Tp,t,fs,delta) - pulse2(t1i-Tp/4,tau,t1i+Tp/4,Tp,t,fs,delta));
...
t11 = -Tp/6;
delta = Tp/1000;
resSig = ModulatedData'.*( pulse(t11,tau,t11+Tp/2,Tp,t,fs,delta) - pulse(t11-Tp/4,tau,t11+Tp/4,Tp,t,fs,delta) );

for i=2:arrayNum
    t1i = (((i-1)*beta*z*sind(theta))/pi-1/3)/2*Tp;
    Sigi = ModulatedData'.*( pulse(t1i,tau,t1i+Tp/2,Tp,t,fs,delta) - pulse(t1i-Tp/4,tau,t1i+Tp/4,Tp,t,fs,delta));
    resSig = resSig + circshift(Sigi, -floor(delta_tt*(i-1)));
end
