...
coeT = 10; %决定间距 
phaseChange = [0 2/3*pi 4/3*pi 2*pi];
%phaseChange = [-2*pi -4/3*pi -2/3*pi 0];
all = 64; %几乘几
thetaAim = 35;
codeLength = 2; %编码长度, 编码长度越长，Theoretical Error越小，但是 Simulation Result 和其 Error 会变得不稳定
...
maxTime = 100;
numberCreature = 10000; %物种数
chance = 60; %交叉概率

numberBit = length(phaseChange);
Dphi = phaseChange(numberBit) - phaseChange(1);
...
Amatrix = asin(coeT.*Dphi/2/pi/numberBit./Amatrix);
usefulA = sum((abs(Amatrix) <= 1));

%均为和法线夹角
thetaAim = thetaAim/180*3.14;
...
finalMatrix = sequenceToMatrix(sequenceH,all);
disp(finalMatrix);
...
function [E2x, E2y, E2z, E3x, E3y, E3z, finalTheta] = positiveSimulation(code, KD, all,phaseChange)
    phase_change = code.*(phaseChange(4) - phaseChange(1))/3-phaseChange(1);
    %amplitude_change = ; %%%%%%%%%%%%%%%%%%%%%
...
function [SH, negTheta] = generateSequence(Amatrix, theta, codeLength, accurate, maxTime, numberCreature, chance, usefulA)
    ...
    toNeg = unidrnd(2,numberCreature,codeLength) - 1;% 0/1
    toNeg = -2.*toNeg + 1; % 1/-1
    codeSetH = codeSetH.*toNeg;
...
function res = digitalMinus(a1,a2)
     res = a1 - a2;
     if a1 + a2 < 0
         res = res + 4;
     end
